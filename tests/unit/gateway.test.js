...
test('connector URLs enforce transport and metadata boundaries without embedding credentials',()=>{
  ...
  assert.throws(()=>validateConnectorBaseUrl('http://169.254.169.254/latest/meta-data','LOCAL_SELF_HOSTED'),e=>e.code==='INVALID_CONNECTOR_URL');
  assert.throws(()=>validateConnectorBaseUrl('http://metadata.google.internal/computeMetadata/v1/','LOCAL_SELF_HOSTED'),e=>e.code==='INVALID_CONNECTOR_URL');
});

test('OpenAI-compatible usage is normalized and untrusted extra usage fields are not persisted',()=>{
  assert.deepEqual(
    extractOpenAICompatibleUsage({usage:{prompt_tokens:4,completion_tokens:3,total_tokens:7,untrusted:'drop-me'}}),
    {inputTokens:4,outputTokens:3,totalTokens:7,rawUsage:{input_tokens:4,output_tokens:3,total_tokens:7}},
  );
  assert.deepEqual(
    extractOpenAICompatibleUsage({usage:{input_tokens:2,output_tokens:5}}),
    {inputTokens:2,outputTokens:5,totalTokens:7,rawUsage:{input_tokens:2,output_tokens:5,total_tokens:7}},
  );
