...
const pool=createPool(); const world='gateway-test'; let system;
...
test('platform-paid inference settles measured usage and idempotent replay never redispatches',async()=>{
  ...
  try{
    ...
    const input={worldId:world,actorEntityId:agent.entity_id,idempotencyKey:'infer-once',descriptorId:descriptor.descriptor_id,connectorId:connector.connector_id,activitySubjectId:agent.entity_id,payerEntityId:agent.entity_id,reservationId,billingDate:'2026-09-16',messages:[{role:'user',content:'hello'}],maxOutputTokens:32,maxChargeMicroE:'1000000'};
...
test('ambiguous timeout becomes OUTCOME_UNKNOWN, retains reservation, and replay does not retry',async()=>{
  ...
  try{
    ...
    const input={worldId:world,actorEntityId:agent.entity_id,idempotencyKey:'timeout-once',descriptorId:descriptor.descriptor_id,connectorId:connector.connector_id,activitySubjectId:agent.entity_id,payerEntityId:agent.entity_id,reservationId,billingDate:'2026-09-16',messages:[{role:'user',content:'slow'}],maxOutputTokens:32,maxChargeMicroE:'1000000'};
...
test('retry is capped and reuses one provider idempotency key only when descriptor allows it',async()=>{
  ...
  try{
    ...
    const out=await infer(pool,{worldId:world,actorEntityId:agent.entity_id,idempotencyKey:'retry-safe',descriptorId:descriptor.descriptor_id,connectorId:connector.connector_id,activitySubjectId:agent.entity_id,payerEntityId:agent.entity_id,reservationId,billingDate:'2026-09-16',messages:[{role:'user',content:'retry'}],maxOutputTokens:32,maxChargeMicroE:'1000000'});
...
test('BYOK sends secret from environment reference, records usage, and does not double-charge Energy',async()=>{
  ...
  try{
    ...
    const out=await infer(pool,{worldId:world,actorEntityId:agent.entity_id,idempotencyKey:'byok',descriptorId:descriptor.descriptor_id,connectorId:connector.connector_id,activitySubjectId:agent.entity_id,payerEntityId:agent.entity_id,billingDate:'2026-09-16',messages:[{role:'user',content:'hello'}],maxOutputTokens:32,maxChargeMicroE:'0'});
...
test('confirmed provider failure with trustworthy usage still settles actual cost',async()=>{
  ...
  try{
    ...
    const out=await infer(pool,{worldId:world,actorEntityId:agent.entity_id,idempotencyKey:'failed-usage',descriptorId:descriptor.descriptor_id,connectorId:connector.connector_id,activitySubjectId:agent.entity_id,payerEntityId:agent.entity_id,reservationId,billingDate:'2026-09-16',messages:[{role:'user',content:'fail'}],maxOutputTokens:32,maxChargeMicroE:'1000000'});
...
test('cross-world descriptor reference is rejected before dispatch',async()=>{
  ...
  try{
    ...
    await assert.rejects(()=>infer(pool,{worldId:'other-world',actorEntityId:other.entity_id,idempotencyKey:'cross-world',descriptorId:descriptor.descriptor_id,connectorId:connector.connector_id,activitySubjectId:other.entity_id,payerEntityId:other.entity_id,billingDate:'2026-09-16',messages:[{role:'user',content:'no'}],maxOutputTokens:16,maxChargeMicroE:'0'}),e=>e.code==='GATEWAY_NOT_AVAILABLE');
