...
import { cancelExecution, getUsageReceipt, inferQuoted, quoteResource, registerConnectorP14, registerDescriptorP14, reserveQuoted } from '../../src/services/p1_4.js';
...
async function configure(baseUrl,{maxRetries=0,supportsIdempotency=false}={}){
  return withTransaction(pool,async(c)=>{
    const descriptor=await registerDescriptorP14(c,{worldId:world,actorEntityId:system.entity_id,descriptorKey:`model-${crypto.randomUUID()}`,modelReference:'p14-model',maxInputTokens:4096,maxOutputTokens:256,timeoutMs:1000,maxRetries,supportsIdempotency,inputRateMicroEPerMillion:'1000000',outputRateMicroEPerMillion:'2000000'});
    const connector=await registerConnectorP14(c,{worldId:world,actorEntityId:system.entity_id,descriptorId:descriptor.descriptor_id,connectorKind:'LOCAL_SELF_HOSTED',billingMode:'PLATFORM_PREPAID',baseUrl});
...
async function quotedReservation(a,descriptor){
  ...
  const reserved=await runCommand(pool,{worldId:world,actorEntityId:a.entity_id,actionType:'economy.reserve',idempotencyKey:`reserve-${crypto.randomUUID()}`,payload:{quoteId:quote.quote_id}},(c,ctx)=>reserveQuoted(c,{worldId:world,entityId:a.entity_id,amountMicroE:quote.max_cost_micro_e,businessKey:`quote-${quote.quote_id}`,quoteId:quote.quote_id,actorEntityId:a.entity_id,actionId:ctx.actionId}));
...
function input(a,descriptor,connector,reservationId,quoteId,overrides={}){
  return{worldId:world,actorEntityId:a.entity_id,idempotencyKey:`infer-${crypto.randomUUID()}`,descriptorId:descriptor.descriptor_id,connectorId:connector.connector_id,activitySubjectId:a.entity_id,payerEntityId:a.entity_id,reservationId,quoteId,billingDate:today,messages:[{role:'user',content:'hello'}],maxOutputTokens:32,...overrides};
}
...
test('platform-paid chain is quote -> reservation -> execution -> receipt -> settlement',async()=>{
  ...
  try{
    ...
    const out=await inferQuoted(pool,input(a,descriptor,connector,reservationId,quote.quote_id));
    assert.equal(out.status,'SUCCEEDED'); assert.equal(out.chargeMicroE,'20');
...
test('strict P1.4 platform inference rejects an unquoted reservation',async()=>{
  ...
  try{
    ...
    await assert.rejects(()=>inferQuoted(pool,input(a,descriptor,connector,reserved.result.reservationId,null)),e=>e.code==='QUOTE_REQUIRED');
...
test('legacy P1.3 descriptor/inference path remains compatible after migration 005',async()=>{
  const p=await provider(async({res})=>success(res));
  try{
    const configured=await withTransaction(pool,async(c)=>{
      const descriptor=await registerDescriptor(c,{worldId:world,actorEntityId:system.entity_id,descriptorKey:`legacy-${crypto.randomUUID()}`,modelReference:'legacy-model',maxInputTokens:4096,maxOutputTokens:256,timeoutMs:1000,maxAttempts:3,supportsIdempotency:true,inputRateMicroEPerMillion:'1000000',outputRateMicroEPerMillion:'2000000'});
      const connector=await registerConnector(c,{worldId:world,actorEntityId:system.entity_id,descriptorId:descriptor.descriptor_id,connectorKind:'LOCAL_SELF_HOSTED',billingMode:'PLATFORM_PREPAID',baseUrl:p.baseUrl});
      return{descriptor,connector};
    });
    const a=await agent('legacy');
    const reserved=await runCommand(pool,{worldId:world,actorEntityId:a.entity_id,actionType:'economy.reserve',idempotencyKey:`legacy-r-${crypto.randomUUID()}`,payload:{}},(c,ctx)=>reserve(c,{worldId:world,entityId:a.entity_id,amountMicroE:'1000000',businessKey:`legacy-${crypto.randomUUID()}`,actorEntityId:a.entity_id,actionId:ctx.actionId}));
    const out=await infer(pool,{worldId:world,actorEntityId:a.entity_id,idempotencyKey:`legacy-i-${crypto.randomUUID()}`,descriptorId:configured.descriptor.descriptor_id,connectorId:configured.connector.connector_id,activitySubjectId:a.entity_id,payerEntityId:a.entity_id,reservationId:reserved.result.reservationId,billingDate:today,messages:[{role:'user',content:'legacy'}],maxOutputTokens:32,maxChargeMicroE:'1000000'});
    assert.equal(out.status,'SUCCEEDED');
    const retry=(await pool.query('SELECT max_attempts,max_retries FROM gateway.capability_descriptors WHERE descriptor_id=$1',[configured.descriptor.descriptor_id])).rows[0];
    assert.equal(retry.max_attempts,3); assert.equal(retry.max_retries,2);
  }finally{await p.close();}
});
