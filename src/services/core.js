...
export async function resolveActor(client, actorEntityId) {
  ...
  const result = await client.query(
    'SELECT entity_id, world_id, entity_type, identity_status FROM core.entities WHERE entity_id = $1',
    [actorEntityId],
  );
...
export async function appendEvent(client, { worldId, aggregateType, aggregateId, eventType, actorEntityId = null, actionId = null, payload = {} }) {
  ...
  const seq = await client.query(
    `SELECT COALESCE(MAX(aggregate_seq), 0) + 1 AS next_seq
       FROM core.life_events
      WHERE world_id = $1 AND aggregate_type = $2 AND aggregate_id = $3`,
    [worldId, aggregateType, aggregateId],
  );
  const inserted = await client.query(
    `INSERT INTO core.life_events
       (world_id, aggregate_type, aggregate_id, aggregate_seq, event_type, actor_entity_id, action_id, payload)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb)
     RETURNING event_id, aggregate_seq, created_at`,
    [worldId, aggregateType, aggregateId, seq.rows[0].next_seq, eventType, actorEntityId, actionId, JSON.stringify(payload)],
  );
  await client.query(
    `INSERT INTO core.outbox (event_id, topic, payload)
     VALUES ($1,$2,$3::jsonb)`,
    [inserted.rows[0].event_id, `nh.${aggregateType.toLowerCase()}.${eventType.toLowerCase()}`, JSON.stringify(payload)],
  );
  ...
  try {
    ...
    const inserted = await client.query(
      `INSERT INTO core.actions (world_id, actor_entity_id, action_type, idempotency_key, payload_hash, status)
       ...
       ON CONFLICT (world_id, actor_entity_id, idempotency_key) DO NOTHING
       RETURNING action_id`,
      [worldId, actorEntityId, actionType, idempotencyKey, payloadHash],
    );

    if (inserted.rowCount === 0) {
      const existing = await client.query(
        `SELECT action_id, action_type, payload_hash, status, result_json, error_code
           FROM core.actions
          WHERE world_id=$1 AND actor_entity_id=$2 AND idempotency_key=$3
          FOR UPDATE`,
        [worldId, actorEntityId, idempotencyKey],
      );
      const row = existing.rows[0];
      if (row.action_type !== actionType || row.payload_hash !== payloadHash) {
        throw Object.assign(new Error('same idempotency key used with different command'), { code: 'IDEMPOTENCY_CONFLICT', status: 409 });
      }
    ...
    actionId = inserted.rows[0].action_id;
    await client.query('SAVEPOINT domain_work');
    try {
      const result = await execute(client, { actionId, actor });
      await client.query(
        `UPDATE core.actions SET status='SUCCEEDED', result_json=$2::jsonb, completed_at=now() WHERE action_id=$1`,
      ...
      await client.query('COMMIT');
      return { replayed: false, actionId, result };
    } catch (error) {
      await client.query('ROLLBACK TO SAVEPOINT domain_work');
      await client.query(
        `UPDATE core.actions SET status='FAILED', error_code=$2, result_json=$3::jsonb, completed_at=now() WHERE action_id=$1`,
        [actionId, error.code || 'COMMAND_FAILED', JSON.stringify({ message: error.message })],
      );
...
export async function createEntity(client, { worldId, entityType, displayId, name, createdBy = null, origin = 'LOCAL' }, context = {}) {
  ...
  const result = await client.query(
    `INSERT INTO core.entities (world_id, entity_type, display_id, name, created_by, origin)
     VALUES ($1,$2,$3,$4,$5,$6)
     RETURNING entity_id, world_id, entity_type, display_id, name, identity_status, created_at`,
    [worldId, entityType, displayId, name, createdBy, origin],
  );
